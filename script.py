...
intro = (''' __  __        _____             _                    _____          
|  \/  |      |  __ \           | |                  |  __ \         
| \  / |_ __  | |  | | _____   _| |     _____   _____| |__) |__ _ __ 
| |\/| | '__| | |  | |/ _ \ \ / / |    / _ \ \ / / _ \  ___/ _ \ '__|
| |  | | | _  | |__| |  __/\ V /| |___| (_) \ V /  __/ |  |  __/ |   
|_|  |_|_|(_) |_____/ \___| \_/ |______\___/ \_/ \___|_|   \___|_| '''
...
def colorText(text):
    for color in colr:
        text = text.replace("[[" + color + "]]", colr[color])
    return text
UI_TYPE = input('terminal or cmd. (t/c): ').lower()
if UI_TYPE == 't':
...
async def main():
    ...
    time.sleep(3)
    done = input(gr+'[+] Members scraped successfully. (Now Press enter key to Add members)')
    await getmem()
